...
if (-not $ScriptPath) {
    ...
    if ($MyInvocation.MyCommand -and $MyInvocation.MyCommand.Path) {
        $candidateRoots += (Split-Path -LiteralPath $MyInvocation.MyCommand.Path -Parent)
    }
    ...
    foreach ($root in $candidateRoots) {
        ...
        $possibleScript = Join-Path $root 'resolution_switch.py'
        if (Test-Path -LiteralPath $possibleScript) {
    ...
    if (-not $ScriptPath) {
        $ScriptPath = 'resolution_switch.py'
    }
}
